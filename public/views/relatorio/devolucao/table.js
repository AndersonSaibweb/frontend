    ...
    try {
      ...
      if (_this.data.length) {
        ...
        try {
                ...
                $(api.column(position).footer()).html(
                  api
                    ...
                    .toLocaleString("pt-br", {
                      style: position == 20 ? "decimal" : "currency",
                      currency: "BRL",
                    })
                );
            ...
            createdRow: function (row, data, dataIndex) {
              ...
              indexRow.forEach((item, index) => {
                ...
                keysNameCase.forEach((nameAlter) => {
                  if (nameAlter == item) {
                    if (
                      _this.case[nameAlter][data[nameAlter]] != undefined &&
                      _this.case[nameAlter][data[nameAlter]] != null
                    ) {
                      $("td", row)
                        .eq(index)
                        .html(_this.case[nameAlter][data[nameAlter]]);
                    }
                  }
                });
                ...
                if (item == "CARTEIRA" && zerarCarteira) {
                  $("td", row).eq(index).html("0");
                }
                if (
                  item == "REPRESENTANTE" ||
                  item == "CLIENTE" ||
          ...
          $("#example1_wrapper").css("padding-top", "16px");
