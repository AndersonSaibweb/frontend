...
class Table {
      ...
      if (_this.data.length) {
        ...
        try {
          var table = $(_this.nameTable).DataTable({
            ...
            //             }, 0).toLocaleString()); //.toLocaleString('pt-br', { style: 'currency', currency: 'BRL' }));
            ...
            rowGroup: {
              ...
              endRender: function (rows, group) {
                ...
                //     line += '<td>' + total_2[group_assoc2].toLocaleString() + '</td>' //.toLocaleString('pt-br', { style: 'currency', currency: 'BRL' })
                // })
                // return $('<tr style="text-align:right; font-weight: bold;" />')
                //     .append(line);
              },
              dataSrc: ["PRODUTO"],
            },
            ...
            createdRow: function (row, data, dataIndex) {
              ...
              indexRow.forEach((item, index) => {
                ...
                if (item == "ESTOQUE_VIRTUAL") {
                  if ($("td", row).eq(index).html() <= 0) {
                    $("td", row).eq(index).css("background-color", "#F9A8A8");
                  }
                }

                if (item == "QTD_POSSIVEL") {
                  $("td", row)
                    .eq(index)
                    .html(
                ...
                if (
                  item == "RESERVA" ||
                  item == "ESTOQUE" ||
                  item == "ESTOQUE_VIRTUAL" ||
                  item == "COMPRAS" ||
                  item == "ESTOQUE_FABRICA" ||
                  item == "ESTOQUE_CONTABIL" ||
                  item == "ESTOQUE_TERCEIRO"
                ) {
                  $("td", row)
                    .eq(index)
                    .html(
  ...
  adicionaFilterProductElementOnDOM(data) {
    function ocultaListaProdutos(oculta) {
      $("#OlProdutosMatPrima").attr("hidden", oculta);
    }

    function definirOpcoesProdutos(listaProdutos) {
      const opcoesProdutos = $("#OlProdutosMatPrima");
      ...
      $("#descriptionFilterMatPrima").focusout(function () {
        setTimeout(() => {
          ocultaListaProdutos(true);
        }, 200);
      });

      $("#OlProdutosMatPrima li").hover(
        function () {
          $(this).css({
      ...
      $("#OlProdutosMatPrima li").css({
        "font-size": ".875rem",
      });

      $("#OlProdutosMatPrima li").click(function defineValorInputFiltro() {
        const produtoSelecionado = $(this)[0].innerHTML;
        $("#descriptionFilterMatPrima")[0].value = produtoSelecionado;
        ocultaListaProdutos(true);
      });
    }
...
`;
    ...
    $("#descriptionFilterMatPrima").keyup(function (ev) {
      if (timeOut) {
