...
function getPedidosStatus() {
  ...
  if (JSON.parse(localStorage.getItem("user")) != null && !request) {
    ...
    ajax(
      _BASE_URL + "api/v1/logistica/getPedidosStatus",
      "GET",
      {},
      function (res) {
        console.log(res);
        if (res.length) {
          if (JSON.parse(localStorage.getItem("user")) != null) {
            ajax(
              _BASE_URL + "api/v1/permissoes/" + "lista",
              "GET",
              {},
              function (grus) {
                perm = grus;
                ...
                res.forEach((element) => {
                  if (element.SITUACAO == APROVADO) {
                    ...
                    let razoaSocial =
                      element.RAZAO_SOCIAL.length >= 45
                        ? element.RAZAO_SOCIAL.substr(0, 45) + ".."
                        : element.RAZAO_SOCIAL;

                    item = `<tr style="background-color:${color}">
                                        <td></td>
                                        <td>${element.COD_PEDIDO}</td>
                                        <td>${dataAnteriorFormatada(
                                          element.DATA_CADASTRO
                                        ).substr(0, 10)}</td>
                                        <td>${element.PESO_BRUTO.toFixed(
                                          2
                                        )}</td>
...
restaurarLiquidacao = (liqu_id) => {
  if (liqu_id != "") {
    ...
    if (JSON.parse(localStorage.getItem("user")) != null) {
      ajax(
        _BASE_URL + _EP_RESTAURAR_LIQUIDACAO,
        "PUT",
        json,
        async function (res) {
          console.log(res);
...
gerarColera = () => {
  ...
  if (selecionados.length > 0 && verifyAgrupamento) {
    ...
    if (JSON.parse(localStorage.getItem("user")) != null) {
      ajax(
        _BASE_URL + _EP_GET_PEDIDOS_BY_AGRUMAMENTO,
        "POST",
        { ID: agrupamento },
        function (res) {
          console.log(res);
