...
function get_all(filtrarFinalizados = true) {
  $("#loading").removeAttr("hidden");
  $("#message").text("Atualizando.. ");

  let url = _BASE_URL + "api/v1/logistica_liquidacao/getLiquidacoes";

  url += `?isFinalizados=${filtrarFinalizados}`;
        ...
        if (!document.getElementById(item.LIQU_ID)) {
          document.getElementById(item.STATUS).innerHTML += `
                            <li class="drag-item" id="${
                              item.LIQU_ID
                            }" ondblclick="openLiqui('${item.LIQU_ID}')">
