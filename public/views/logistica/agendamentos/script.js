...
atualizarTabela = () => {
  if (JSON.parse(localStorage.getItem("user")) != null && !request && !edit) {
    ...
    ajax(
      _BASE_URL + _EP_GET_AGENDAMENTOS,
      "GET",
      {},
      function (res) {
        console.log(res);
        ...
        if (res.length) {
          if (JSON.parse(localStorage.getItem("user")) != null) {
            ajax(
              _BASE_URL + _EP_GET_PERMISSOES + "lista",
              "GET",
              {},
              function (grus) {
                console.log(grus);
                ...
                try {
                  document
                    .getElementById("custom-tabs-four-editar-tab")
                    .remove();
                } catch (e) {}
                // }
                ...
                res.forEach((element) => {
                  if (element.Status != "E") {
                    ...
                    if (element.Status != "" && element.Status != undefined) {
                      if (
                        element["Hora Status"] != "" &&
                        element["Hora Status"] != undefined
                      ) {
                        hr = element["Hora Status"].substr(0, 5);
                      }
                      if (
                        element["Data Status"] != "" &&
                        element["Data Status"] != undefined
                      ) {
                ...
                if (!starting) {
                  ...
                  $("#example1").on("dblclick", "tbody tr", function () {
                    ...
                    if (
                      perm.includes("viewUpdateCarga") &&
                      rowsEdit[11].innerHTML != "0"
                    ) {
                      if (
                        rowsEdit[9].innerHTML == "CHEGADA NA PORTARIA" ||
                        rowsEdit[9].innerHTML == "AGUARDANDO VEÍCULO" ||
                        rowsEdit[9].innerHTML == "OCORRENCIA"
                      ) {
                        if (edit) return;
                        edit = true;
                        ...
                        let arroptFrete =
                          document.getElementById("optFrete").options;
                        ...
                        try {
                          document.querySelector(
                            'span[aria-labelledby="select2-optTransportadoras-container"]'
                        ...
                        if (freteType == "FOB") {
                          ...
                          document
                            .getElementById("txtTransportadoraa")
                            .select();
                        } else {
                  ...
                  $.fn.dataTable.ext.search.push(function (
                    ...
                    if (filtroPesquisa != "" && filtroPesquisa != undefined) {
                      if (
                        data[0]
                          .toUpperCase()
                          .includes(filtroPesquisa.toUpperCase())
                      ) {
                        return true;
                      }
                      if (
  ...
  if (edit && perm.includes("viewUpdateCarga")) {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        json["frete"] =
          optFrete.options[optFrete.selectedIndex].value.toUpperCase()[0]; //rowsEdit[1].querySelector('input').value.toUpperCase()[0]
        json["placa"] = rowsEdit[2].querySelector("input").value.toUpperCase();
        json["motorista"] = rowsEdit[3]
          .querySelector("input")
          .value.toUpperCase();
        json["dataColeta"] = rowsEdit[7]
          .querySelector("input")
          .value.toUpperCase();
        json["horaColeta"] = rowsEdit[8]
          .querySelector("input")
          .value.toUpperCase();

        if (JSON.parse(localStorage.getItem("user")) != null) {
          ajax(
            _BASE_URL + _EP_UPDATE_CARGA,
            "PUT",
            json,
            function (res) {
              if (res.status == 200) {
    ...
    if (idFrota == "27998499cvjk") {
      ...
      switch (status) {
        case "A":
          ...
          try {
            document.getElementById("viewChegadaPortaria").remove();
          } catch (e) {}
          break;
        case "B":
          ...
          try {
            document.getElementById("viewEntradaLiberada").remove();
          } catch (e) {}
          break;
        case "C":
          ...
          try {
            document.getElementById("viewCarregamentoIniciado").remove();
          } catch (e) {}
          break;
        case "D":
          ...
          try {
            document.getElementById("viewCarregamentoFinalizado").remove();
          } catch (e) {}
          break;
        case "E":
          $("#divRegistrar").removeAttr("hidden");
      ...
      try {
        perm.includes("viewChegadaPortaria")
          ? null
          : document.getElementById("viewChegadaPortaria").remove();
      } catch (err) {}
      try {
        perm.includes("viewEntradaLiberada")
          ? null
          : document.getElementById("viewEntradaLiberada").remove();
      } catch (err) {}
      try {
        perm.includes("viewCarregamentoIniciado")
          ? null
          : document.getElementById("viewCarregamentoIniciado").remove();
      } catch (err) {}
      try {
        perm.includes("viewCarregamentoFinalizado")
          ? null
          : document.getElementById("viewCarregamentoFinalizado").remove();
      } catch (err) {}
      try {
        perm.includes("viewSaidaLiberada")
          ? null
          : document.getElementById("viewSaidaLiberada").remove();
      } catch (err) {}
      try {
        perm.includes("viewOcorrencia")
          ? null
          : document.getElementById("viewOcorrencia").remove();
      } catch (err) {}
      ...
      $("#modal-default2 .timeline").animate(
        { scrollTop: $("#modal-default2 .timeline").offset().top },
        "slow"
      );
...
registrar = () => {
  ...
  if (validarCamposInputs(obrigatorio)) {
    ...
    console.log(idLiquidacao);
    console.log(idColeta);
    let status = document.getElementById("optStatus").value;
    ...
    atualizarStatusAgendamento(status);
  }
};
...
getLogsAgendamento = () => {
  if (idLiquidacao != 0 && idColeta != 0) {
    ...
    if (JSON.parse(localStorage.getItem("user")) != null && !request) {
      ajax(
        _BASE_URL + _EP_GET_CDTR_LOG_AGENDAMENTOS,
        "POST",
        json,
        function (res) {
          if (res.length) {
            ...
            res.forEach((item) => {
              ...
              switch (item.CDTR_STATUS) {
                ...
                case "C":
                  try {
                    document
                      .getElementById("viewCarregamentoIniciado")
                      .remove();
                  } catch (e) {}
                  break;
                case "D":
                  ...
                  try {
                    document
                      .getElementById("viewCarregamentoFinalizado")
                      .remove();
                  } catch (e) {}
                  break;
                case "E":
...
function mudandoFrete() {
  ...
  if (optFrete.options[optFrete.selectedIndex].value == "CIF") {
    ...
    try {
      document.getElementById("select2-optTransportadoras-container").style =
        "font-size: 14px;line-height: 1.8 !important";
    } catch (e) {}
...
abrirChamado = () => {
  if (perm.includes("viewAberturaChamado")) {
    let txtChamadoTransportadora = document.getElementById(
    ...
    let txtChamadoCpfMotorista = document.getElementById(
      "txtChamadoCpfMotorista"
    );
    ...
    if (
      !validaCnpjCpf(document.getElementById("txtChamadoCpfMotorista").value)
    ) {
      document.getElementById(
        "txtChamadoCpfMotorista"
      ).style = `border: 1px solid #ff0000;`;
