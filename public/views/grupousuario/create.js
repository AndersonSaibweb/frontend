...
function createGrupoUsuario(base64String) {
  ...
  permList.forEach((perm) => {
    permListText == ""
      ? (permListText += perm.id + ":" + perm.checked)
      : (permListText += ";" + perm.id + ":" + perm.checked);
  });
...
function recuperarDados(id) {
  ...
  if (JSON.parse(localStorage.getItem("user")) != null) {
    ajax(
      _BASE_URL + _EP_GET_ID_GRUPOUSUARIO + dadosUrl.id,
      "GET",
      {},
      function (grus) {
        if (grus) {
          ...
          try {
            if (grus.GRUS_PERMISSAO.length > 0) {
              // var enc = new TextDecoder("utf-8");
              // var arr = new Uint8Array(grus.GRUS_PERMISSAO);
              // console.log("arr", arr);
              let perm = grus.GRUS_PERMISSAO;
              // perm.split(`;`);
