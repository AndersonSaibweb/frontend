      ...
      function (res) {
        ...
        if (res.length) {
          ...
          res.forEach((element) => {
            ...
            let nomeGrupo =
              element.ID_GRUPOUSUARIO == 0 ? "Sem Grupo" : element.GRUS_NOME;
...
function getUserId(userId) {
  ...
  ajax(
    _BASE_URL + "api/v1/usuario/getUserId",
    "POST",
    { id: userId },
    function (res) {
      console.log(res);
...
salvar = () => {
  ...
  if (JSON.parse(localStorage.getItem("user")) != null) {
    ajax(
      _BASE_URL + _EP_UPDATE_GRUS_USUARIO + idUser,
      "PUT",
      json,
      function (res) {
        getAllUsuario();
