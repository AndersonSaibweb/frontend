      ...
      res.forEach((linha) => {
        ...
        let item =
          `<tr style='background-color:${cor}'>
                                ...
                                <td class='itens'>${linha["PESO"].toFixed(
                                  3
                                )}</td>
                                <td class='itens'>` +
          getDataFormatada(linha["DATA_COLETA"]).substring(0, 10) +
          `</td>
...
function aoAlterarSelectCliente(ev) {
  console.log(ev);
}

function getClientes() {
  const selectOpcoesClientes = $("#clientes");
  $(selectOpcoesClientes).change(function (ev) {
...
function can_fat(id = 0) {
  Swal.fire({
    ...
    if (result.isConfirmed) {
      ajax(
        _BASE_URL + "api/v1/expedicao/can_fat",
        "POST",
        { liquidacao: id },
        function (res) {
          console.log(res);
          if (res.status) {
            ...
            Swal.fire("Algo deu errado!", res.message, "error");
            get_all();
          }
        }
      );
...
function verificar_estoque(id = 0, id_liqui = 0) {
  ...
  Swal.fire({
    ...
    didOpen: () => {
      ...
      ajax(
        _BASE_URL + "api/v1/expedicao/verificar_estoque",
        "POST",
        { coleta: id },
        function (res) {
          console.log(res);
          ...
          if (res.length) {
            ...
            if (id_liquidacao) {
              opc =
                `<button onclick="gerar_palete(${id_liquidacao})" style="margin-right: 50px;" class="btn btn-success">Liberar</button>` +
                opc;
            }
